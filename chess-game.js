...
// VERSION: 2.4.2 - Rewritten minimax: proper alpha-beta with SEE pruning and mate detection
...
// ========== MATE SCORES ==========
// Convention: score is always from the AI's perspective.
// AI_MATE_BASE is added when AI delivers mate; MATE_BASE when AI is mated.
// The +depth bonus ensures faster mates are preferred.
const MATE_SCORE = 100000;
const INF = 1000000;

function mateScoreForAI(depthRemaining) { return MATE_SCORE - depthRemaining; }
function mateScoreAgainstAI(depthRemaining) { return -MATE_SCORE + depthRemaining; }
...
function resetSearchHeuristics() {
    for (let i = 0; i < 64; i++) { killerMoves[i][0] = null; killerMoves[i][1] = null; }
    historyTable.clear();
}
...
function recordHistory(move, depth) {
    ...
    historyTable.set(moveKey, (historyTable.get(moveKey) || 0) + depth * depth);
}
...
class PersistentMoveTree {
    ...
    getMoveKey(fromRow, fromCol, toRow, toCol) { return `${fromRow},${fromCol},${toRow},${toCol}`; }
...
function getEndgameCheckPenalty(boardState, player) {
    ...
    for (let row = 0; row < 8; row++) {
        for (let col = 0; col < 8; col++) {
            ...
            if (piece === '♙' && player === 'white') {
                if (row === 1) score += 400; else if (row === 2) score += 200; else if (row === 3) score += 80; else if (row === 4) score += 30;
            }
            if (piece === '♟' && player === 'black') {
                if (row === 6) score += 400; else if (row === 5) score += 200; else if (row === 4) score += 80; else if (row === 3) score += 30;
            }
        }
    }
...
function evaluatePositionForSearch(boardState, player, moveNumber) {
    ...
    evaluation += getEndgameCheckPenalty(boardState, player);
...
// ========== MINIMAX — REWRITTEN ==========
// Convention:
//   - Score is ALWAYS from the perspective of `aiColor`.
//   - The root always maximises from the AI's perspective.
//   - At every node, we determine whether `aiColor` is the one to move.
//     If yes, we maximise; if no, we minimise.
//   - Mate scores use MATE_SCORE ± depth so faster mates win.
...
let aiColorForSearch = 'black';

function minimax(boardState, depth, alpha, beta, playerToMove, depthFromRoot) {
    // ---------- Terminal detection (at any depth) ----------
    const moves = getAllPossibleMovesForPosition(boardState, playerToMove);
    
    if (moves.length === 0) {
        if (isKingInCheckForPosition(boardState, playerToMove)) {
            // Mate. Score from AI's perspective.
            if (playerToMove === aiColorForSearch) {
                // AI is mated — very bad.
                return mateScoreAgainstAI(depthFromRoot);
            } else {
                // Opponent is mated — very good.
                return mateScoreForAI(depthFromRoot);
            }
        }
        return 0; // Stalemate
    }
    ...
    const inCheck = isKingInCheckForPosition(boardState, playerToMove);
    if (depth <= 0 && !inCheck) {
        return quiescence(boardState, alpha, beta, playerToMove, SEARCH_CONFIG.quiescenceDepth);
    }
    if (depth <= 0 && inCheck) {
        // Check extension — search at least one more ply
        depth = 1;
    }
    
    // ---------- Order moves ----------
    moves.sort((a, b) => scoreMoveForOrdering(boardState, b, playerToMove, depth) - scoreMoveForOrdering(boardState, a, playerToMove, depth));
    
    const isMaximizing = (playerToMove === aiColorForSearch);
    const nextPlayer = playerToMove === 'white' ? 'black' : 'white';
    
    if (isMaximizing) {
        let best = -INF;
        for (const move of moves) {
            // SEE pruning: at depth > 1, skip clearly losing captures
            const targetPiece = boardState[move.toRow][move.toCol];
            if (targetPiece && depth > 1) {
                ...
                if (!givesCheck) {
                    const see = evaluateCaptureSafety(boardState, move.fromRow, move.fromCol, move.toRow, move.toCol, playerToMove);
                    if (see < -100) continue;
                }
            }
            
            const newBoard = makeTestMoveForPosition(boardState, move.fromRow, move.fromCol, move.toRow, move.toCol);
            const score = minimax(newBoard, depth - 1, alpha, beta, nextPlayer, depthFromRoot + 1);
            
            if (score > best) best = score;
            if (score > alpha) alpha = score;
            if (alpha >= beta) {
                if (!targetPiece) { recordKillerMove(depth, move); recordHistory(move, depth); }
                break;
            }
        }
        return best;
...
function quiescence(boardState, alpha, beta, playerToMove, qDepth) {
    // Terminal check
    const moves = getAllPossibleMovesForPosition(boardState, playerToMove);
    if (moves.length === 0) {
        if (isKingInCheckForPosition(boardState, playerToMove)) {
            if (playerToMove === aiColorForSearch) return mateScoreAgainstAI(0);
            return mateScoreForAI(0);
        }
        return 0;
    }
    
    // Stand pat — score is from AI's perspective, so we need to convert.
    // evaluatePositionForSearch(boardState, player) scores from `player`'s perspective.
    // We want from AI's perspective, so we call with aiColorForSearch.
    let standPat = evaluatePositionForSearch(boardState, aiColorForSearch, moveCount);
    ...
    const candidateMoves = inCheck
        ? moves
        : moves.filter(m => boardState[m.toRow][m.toCol] !== '');
    ...
    if (isMaximizing) {
        ...
        for (const move of candidateMoves) {
            ...
            const newBoard = makeTestMoveForPosition(boardState, move.fromRow, move.fromCol, move.toRow, move.toCol);
            const score = quiescence(newBoard, alpha, beta, nextPlayer, qDepth - 1);
            if (score > best) best = score;
            if (score > alpha) alpha = score;
            if (alpha >= beta) break;
        }
...
function findBestMoveWithRiskAssessment() {
    ...
    // Root perspective is always the AI
    aiColorForSearch = currentPlayer;
    ...
    // Order moves by SEE before searching
    allMoves.sort((a, b) => {
    ...
    for (const move of allMoves) {
        ...
        // Search this move with the opponent to move at the next ply
        const score = minimax(newBoard, searchDepth - 1, -INF, INF, opponentColor, 1);
        
        evaluatedMoves.push({ move, score });
    }
    
    if (evaluatedMoves.length === 0) {
        console.log("⚠️ No moves evaluated");
        return allMoves[0];
    }
    
    // ALWAYS maximize from AI's perspective
    evaluatedMoves.sort((a, b) => b.score - a.score);
    
    const bestMove = evaluatedMoves[0];
    const searchTime = (performance.now() - searchStartTime).toFixed(0);
    const moveStr = toAlgebraicMove(bestMove.move.fromRow, bestMove.move.fromCol, bestMove.move.toRow, bestMove.move.toCol);
    console.log(`⏱️ ${searchTime}ms | Selected: ${moveStr} | Score: ${bestMove.score} | Cache: ${LAYER_HITS}h/${LAYER_MISSES}m`);
    ...
    for (let r = 0; r < 8; r++) {
        for (let c = 0; c < 8; c++) {
            if (isValidMove(row, col, r, c)) {
                ...
                if (square) {
                    if (board[r][c] && isPlayerPiece(board[r][c], currentPlayer === 'white' ? 'black' : 'white')) square.classList.add('capture-move');
...
function isValidMove(fromRow, fromCol, toRow, toCol) {
    ...
    if ((piece === '♔' || piece === '♚') && Math.abs(toCol - fromCol) === 2 && fromRow === toRow) return canCastle(fromRow, fromCol, toRow, toCol);
    const pieceCode = pieceMap[piece];
...
function canCastle(fromRow, fromCol, toRow, toCol) {
    ...
    for (let col = start; col < end; col++) if (board[fromRow][col] !== '') return false;
...
function isKingInCheck(testBoard, player) { return isKingInCheckForPosition(testBoard, player); }
...
function makeMove(fromRow, fromCol, toRow, toCol) {
    ...
    board[toRow][toCol] = piece;
    board[fromRow][fromCol] = '';
    if ((piece === '♙' && toRow === 0) || (piece === '♟' && toRow === 7)) board[toRow][toCol] = piece === '♙' ? '♕' : '♛';
    enPassantTarget = null;
    if ((piece === '♙' || piece === '♟') && Math.abs(toRow - fromRow) === 2) enPassantTarget = { row: fromRow + (toRow - fromRow) / 2, col: fromCol };
...
function getAllPossibleMoves(player) { return getAllPossibleMovesForPosition(board, player); }
...
function parseAlgebraicMove(moveStr) {
    ...
    if (fromRow < 0 || fromRow > 7 || fromCol < 0 || fromCol > 7 || toRow < 0 || toRow > 7 || toCol < 0 || toCol > 7) return null;
...
window.addEventListener('load', function() {
    ...
    if (typeof GamePatternLearner !== 'undefined') {
        ...
        fetch('games.csv').then(response => {
            if (!response.ok) throw new Error('CSV not found');
            return response.text();
        }).then(csvText => patternLearner.loadFromCSV(csvText)).then(loaded => {
...
function newGame() {
    ...
    const syncStatusElement = document.getElementById('sync-status');
    if (thinkingElement) thinkingElement.style.display = 'none';
    if (syncStatusElement) { syncStatusElement.textContent = 'Ready'; syncStatusElement.classList.remove('thinking'); }
...
console.log(`✅ Chess Game v${GAME_VERSION} loaded - Minimax rewritten`);
