// chess-game.js
// VERSION: 2.4.2 - Full move evaluation, no pruning except alpha-beta
...
// ========== MOVE ORDERING (pure heuristic, no pruning) ==========
...
// ========== SEE (used only for move ordering, not for pruning) ==========
...
// ========== QUIESCENCE (only captures, only at depth 0) ==========
// The purpose of quiescence is to finish tactical exchanges at the horizon,
// not to prune. It still looks at every capture until the position is quiet.
function quiescenceSearch(boardState, alpha, beta, player, qDepth) {
    ...
    // If in check, consider all evasions. Otherwise, only captures.
...
// ========== MINIMAX — looks at every move, no pruning except alpha-beta ==========
function minimaxWithRisk(boardState, depth, alpha, beta, isMaximizingPlayer, player, moveNumber, trackWorstCase = false) {
    ...
    // At the horizon: use quiescence to finish the tactical exchange cleanly.
    if (depth <= 0 && !inCheck) {
        return quiescenceSearch(boardState, alpha, beta, player, SEARCH_CONFIG.quiescenceDepth);
    }
    if (depth <= 0) depth = 1;

    // Order moves by heuristic (captures first, then killers, then history).
    // Ordering does not skip moves — it only makes alpha-beta cutoffs happen sooner.
    ...
    if (isMaximizingPlayer) {
        ...
        for (const move of moves) {
            // NO pruning. Every move is searched.
            const newBoard = makeTestMoveForPosition(boardState, move.fromRow, move.fromCol, move.toRow, move.toCol);
            ...
            if (beta <= alpha) {
                // Alpha-beta cutoff — mathematically safe, does not change the result.
                const targetPiece = boardState[move.toRow][move.toCol];
...
// ========== RISK ASSESSMENT (kept for API compatibility) ==========
...
function findBestMoveWithRiskAssessment() {
    ...
    for (const move of allMoves) {
        ...
        if (newBoard && isKingInCheckForPosition(newBoard, opponentColor)) {
            ...
            if (opponentMoves.length === 0) {
                console.log(`👑 Immediate mate: ${toAlgebraicMove(move.fromRow, move.fromCol, move.toRow, move.toCol)}`);
                return move;
            }
        }
    }
    ...
    // Order root moves by heuristic
    allMoves.sort((a, b) => scoreMoveForOrdering(board, b, currentPlayer, searchDepth) - scoreMoveForOrdering(board, a, currentPlayer, searchDepth));

    const evaluatedMoves = [];
    for (const move of allMoves) {
        const newBoard = makeTestMoveForPosition(board, move.fromRow, move.fromCol, move.toRow, move.toCol);
        const bestResult = minimaxWithRisk(newBoard, searchDepth - 1, -Infinity, Infinity, false, opponentColor, moveCount + 1, false);
        const worstResult = minimaxWithRisk(newBoard, searchDepth - 1, -Infinity, Infinity, false, opponentColor, moveCount + 1, true);
    ...
    // Sort so the AI always maximizes from its own perspective
    ...
    let bestSafeMove;
    if (currentPlayer === 'black') {
        bestSafeMove = riskAssessed.reduce((best, current) => current.bestCase < best.bestCase ? current : best, riskAssessed[0]);
    } else {
...
function createBoard() {
    ...
    for (let row = 0; row < 8; row++) {
        for (let col = 0; col < 8; col++) {
            ...
            if (lastMove && ((lastMove.fromRow === row && lastMove.fromCol === col) || (lastMove.toRow === row && lastMove.toCol === col))) square.classList.add('last-move');
            const piece = board[row][col];
            if ((piece === '♔' && isKingInCheck(board, 'white')) || (piece === '♚' && isKingInCheck(board, 'black'))) square.classList.add('in-check');
...
function updateMoveHistory() {
    ...
    for (let i = 0; i < moveHistory.length; i += 2) {
        ...
        formattedMoves.push(`${moveNumber}. ${moveHistory[i] || ''} ${moveHistory[i + 1] || ''}`);
    }
...
function makeAIMove() {
    ...
    if (syncStatusElement) { syncStatusElement.textContent = `AI (${aiPlayer}) thinking...`; syncStatusElement.classList.add('thinking'); }
    setTimeout(() => {
...
window.addEventListener('load', function() {
    ...
    if (typeof GamePatternLearner !== 'undefined') {
        ...
        fetch('games.csv').then(r => { if (!r.ok) throw new Error('CSV not found'); return r.text(); })
            .then(csvText => patternLearner.loadFromCSV(csvText))
            .then(loaded => { console.log(`🧠 Pattern Learner: Loaded ${loaded} games`); updateAIStats(); })
...
console.log(`✅ Chess Game v${GAME_VERSION} loaded - Every move searched, no pruning`);
