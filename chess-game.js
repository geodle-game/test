...
// VERSION: 2.4.2 - Checkmate Detection + Quiescence Fix + Root Mate Net + Pruning
...
function minimaxWithRisk(boardState, depth, alpha, beta, isMaximizingPlayer, player, moveNumber, trackWorstCase = false) {
    ...
    if (isMaximizingPlayer) {
        ...
        for (const move of moves) {
            ...
            // Exemption: never prune a capture that gives check (it might be a forcing mate).
            if (targetPiece) {
...
function findBestMoveWithRiskAssessment() {
    ...
    // Root alpha-beta: after scoring each move, update alpha for siblings.
    // Since the minimizer returns a value from the opponent's perspective
    // for the maximizing AI, we track the best score found so far and pass
    // it as alpha to prune sibling subtrees.
...
console.log(`✅ Chess Game v${GAME_VERSION} loaded - Pruning active, smartness intact`);
